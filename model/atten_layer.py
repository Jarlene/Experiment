...
class Attention(Layer):
    ...
    def build(self, input_shape):
        ...
        if 'local-p' in self.alignment_type:
            ...
            self.W_p.build(input_shape=(None, None, self.hidden_dim)
                           )                               # (B, 1, H)
    ...
    def call(self, inputs):
        ...
        target_hidden_state = tf.expand_dims(
            input=target_hidden_state, axis=1)                     # (B, 1, H)
        ...
        if self.alignment_type == 'global':
            ...
            if self.alignment_type == 'local-m':
                ...
                source_hidden_states = Lambda(lambda x: x[:, left:right, :])(
            ...
            # Completely Predictive Alignment
                ...
                aligned_position = tf.squeeze(
                    aligned_position, axis=-1)                            # (B, S)
        ...
        if 'dot' in self.score_function:
            attention_score = Dot(axes=[2, 2])(
                [source_hidden_states, target_hidden_state])         # (B, S*, 1)
        ...
        # General Score Function
            ...
            weighted_hidden_states = self.W_a(
                source_hidden_states)                                 # (B, S*, H)
            attention_score = Dot(axes=[2, 2])(
                [weighted_hidden_states, target_hidden_state])       # (B, S*, 1)

        # Location-based Score Function
        elif self.score_function == 'location':
            weighted_target_state = self.W_a(
                target_hidden_state)                                   # (B, 1, H)
            attention_score = Activation('softmax')(
                weighted_target_state)                          # (B, 1, H)
            attention_score = RepeatVector(source_hidden_states.shape[1])(
                attention_score)          # (B, S*, H)
            attention_score = tf.reduce_sum(
                attention_score, axis=-1)                               # (B, S*)
            attention_score = tf.expand_dims(
                attention_score, axis=-1)                              # (B, S*, 1)
        ...
        # Concat Score Function
            ...
            weighted_target_state = self.U_a(
                target_hidden_state)                                   # (B, 1, H)
            weighted_sum = weighted_hidden_states + \
                weighted_target_state                           # (B, S*, H)
            # (B, S*, H)
            weighted_sum = Activation('tanh')(weighted_sum)
            # (B, S*, 1)
            attention_score = self.v_a(weighted_sum)

        # Compute attention weights
        attention_weights = Activation('softmax')(
            attention_score)                                  # (B, S*, 1)

        # Distribute weights around aligned position for local-p approach only
        # Gaussian Distribution
        if self.alignment_type == 'local-p':
            def gaussian_estimation(s): return tf.exp(-tf.square(s - aligned_position) /
            ...
            for i in range(1, self.input_sequence_length):
                gaussian_factor = Concatenate(axis=1)(
                    [gaussian_factor, gaussian_estimation(i)])    # (B, S*, 1)
            attention_weights = attention_weights * \
                gaussian_factor                                 # (B, S*, 1)

        # Derive context vector
        context_vector = source_hidden_states * \
            attention_weights                                   # (B, S*, H)
...
class SelfAttention(Layer):
    ...
    def call(self, inputs):  # (B, S, H)
        ...
        hidden_states_transposed = Permute(dims=(2, 1))(
            inputs)                                     # (B, H, S)
        # (B, size, S)
        attention_score = tf.matmul(W1, hidden_states_transposed)
        attention_score = Activation('tanh')(
            attention_score)                                       # (B, size, S)
        # (B, num_hops, S)
        ...
        if self.use_penalization:
            ...
            # (B, num_hops, num_hops)
            product = tf.matmul(attention_weights,
                                attention_weights_transposed)
            # (B, num_hops, num_hops)
            identity = tf.eye(self.num_hops, batch_shape=(inputs.shape[0],))
            frobenius_norm = tf.sqrt(tf.reduce_sum(
                tf.square(product - identity)))  # distance
